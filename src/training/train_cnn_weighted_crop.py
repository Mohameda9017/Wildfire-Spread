...
def train_one_epoch(model, loader, optimizer, device, pos_weight=5.0, neg_weight=1.0):
    model.train()
    total_loss = 0.0
...
@torch.no_grad()
def evaluate(model, loader, device, pos_weight=5.0, neg_weight=1.0):
    model.eval()
    total_loss = 0.0
...
def main():
    ...
    train_dataset = WildfireDataset(
        ...
        transform=RandomCropPair(crop_size=crop_size),
    )
    ...
    print(f"Training with random crop size: {crop_size}")
    print(f"Using weighted masked BCE: pos_weight={pos_weight}, neg_weight={neg_weight}")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if eval_loss < best_eval_loss:
            ...
            save_path = checkpoint_dir / "simple_cnn_weighted_crop_best.pt"
